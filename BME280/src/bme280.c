...
void writeSensorData(struct bme280_data *comp_data);
void writeToCSV(float temp, float press, float hum);
...
int main(int argc, char* argv[])
{
    ...
    // need to supply i2c bus to read device from
    if (argc < 2)
    {
    ...
    while (1) {
        ...
        int seconds_until_next_zero = 60 - time_info->tm_sec;
        printf("%d seconds until activation\n", seconds_until_next_zero);
        
        // Wait until the start of the next minute
        sleep(seconds_until_next_zero);

        // collect data then sleep for SLEEP duration
        while (1) {
            time(&current_time);
            time_info = localtime(&current_time);
            ...
            sleep(10); // Sleep for 10 seconds before the next call
        }
    }
...
void writeSensorData(struct bme280_data *comp_data)
{
    ...
    writeToCSV(temp, press, hum);
}

void writeToCSV(float temp, float press, float hum) {
    /* open file in write mode */
    FILE *file = fopen("../../AQMGUI/data/BME280DATA.csv", "w");
    ...
    /* write to file */
    fprintf(file, "%0.2lf, %0.2lf, %0.2lf, %s\n", temp, press, hum, ctime(&current_time));
